...
type ProxmoxClient interface {
	...
	AttachCloudInitDrive(ctx context.Context, node string, vmid int, slot, storage string) error
...
type StandaloneNetService interface {
	...
	// the migrate outcome takes precedence.
...
func (s *Service) Provision(ctx context.Context, req Request, progress ProgressReporter) (*Result, error) {
	...
	// Reject pre-D-boot templates up-front. Cloning would technically
	// succeed but the resulting VM has no qemu-guest-agent (cidata-based
	// install path is gone) and WaitForIP's agent loop would silently
	// time out. Surface a clear "re-run bootstrap" message instead.
	if err := s.verifyTemplateBaked(ctx, target, templateVMID); err != nil {
		return nil, err
	}
	...
	// Attach Proxmox's managed cloud-init drive on the same storage as
	// the boot disk. Drive contents are regenerated by PVE on every
	// SetCloudInit call (and on first boot) from the VM's config fields.
	// qemu-guest-agent is already baked into the template via the bake
	// ceremony (see bootstrap/bake.go), so the managed drive only has to
	// carry network / user / SSH — no custom user-data needed.
	consolePassword := generateConsolePassword()
	managedCI := s.installManagedCloudInit(ctx, target, newVMID, consolePassword)
	...
	res.ConsolePassword = managedCI.ConsolePassword
	res.CloudInitError = managedCI.Error
	return res, nil
}
...
func (s *Service) deleteVM(ctx context.Context, vm *db.VM) error {
	...
	// Clean up any legacy per-VM cidata ISO. Best-effort — new VMs
	// don't have one (they use the managed cloudinit drive that
	// Proxmox cleans up automatically on destroy), and legacy VMs may
	// have already had theirs swept by an earlier migrate/renumber
	// swap. DeleteStorageVolume swallows 404 so re-calls are safe.
	s.deleteLegacyCIDataISO(ctx, vm.Node, vm.VMID)
